...
const Prize = () => {
	...
	if (id != undefined) {
		prize = prizes.find((prize) => prize.id !== undefined && prize.id === parseInt(id));
	}
			...
			{prize != undefined ? (
				<div className="flex justify-center h-screen font-inter">
					...
					<div className="px-16 py-8">
						<h1 className="text-4xl text-green-600 break-words w-72 font-bold mb-3">{prize.title}</h1>
						<p className="text-sm text-gray-600 mb-6">Prazo: 20/12</p>
						<div className="mb-2">
							<ProgressBar progress={calculatePorcentage(prize.points)} height={"h-6"} points={user.points} />
						</div>
						<p className="text-xs text-gray-400 break-words mb-6">Necessários {prize.points.toLocaleString("pt-BR")} pontos</p>
